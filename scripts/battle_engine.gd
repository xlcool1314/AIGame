...
func _apply_actions(actions: Array, from_player: bool) -> void:
	for action in actions:
		...
		match action_type:
			"damage":
				if from_player:
					var dmg_to_enemy := max(value - enemy_block, 0)
					enemy_block = max(enemy_block - value, 0)
					enemy_hp -= dmg_to_enemy
					emit_signal("combat_log", "造成 %d 伤害（敌人格挡剩余 %d）" % [dmg_to_enemy, enemy_block])
				else:
					var dmg_to_player := max(value - player_block, 0)
					player_block = max(player_block - value, 0)
