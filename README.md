# AIGame（Godot 4 + C# 数据驱动卡牌战斗原型）

<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
这是一个基于 **Godot 4 + C#** 的扫雷 + 卡牌战斗探索原型。核心跑局围绕矿区扫雷探勘展开：玩家在分层矿井中选择路线，进入矿区后翻格、标记暗雷、依据数字判断风险；战斗、事件、商店和休整作为路线中的风险与成长节点。
=======
这是一个基于 **Godot 4 + C#** 的最小可运行战斗原型，提供类似《杀戮尖塔》的单场战斗循环：抽牌、出牌、结束回合、敌人行动。
>>>>>>> theirs
=======
这是一个基于 **Godot 4 + C#** 的最小可运行战斗原型，提供类似《杀戮尖塔》的单场战斗循环：抽牌、出牌、结束回合、敌人行动。
>>>>>>> theirs
=======
这是一个基于 **Godot 4 + C#** 的最小可运行战斗原型，提供类似《杀戮尖塔》的单场战斗循环：抽牌、出牌、结束回合、敌人行动。
>>>>>>> theirs
=======
这是一个基于 **Godot 4 + C#** 的最小可运行战斗原型，提供类似《杀戮尖塔》的单场战斗循环：抽牌、出牌、结束回合、敌人行动。
>>>>>>> theirs
=======
这是一个基于 **Godot 4 + C#** 的最小可运行战斗原型，提供类似《杀戮尖塔》的单场战斗循环：抽牌、出牌、结束回合、敌人行动。
>>>>>>> theirs

## 当前实现

- 全部核心逻辑使用 C#：`GameData.cs` / `BattleEngine.cs` / `BattleScene.cs`
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
- 数据驱动：卡牌 / 敌人 / 初始牌组 / 事件 / 奖励来源于 JSON 配置
- 矿井探索：分层路线选择包含扫雷矿区、事件、战斗、商店、休整和最终精英战
- 扫雷核心：矿区房间支持入口、出口、怪物、陷阱、宝箱、矿石、翻格、标记和安全区揭示
- 双线索规则：揭示安全格会显示 `D` 危险数与 `R` 奖励数，分别提示周围怪物/陷阱与宝箱/矿石数量
- 出口规则：找到出口后可进入下一层，也可以继续探索获取更多奖励
- 主界面流程：从主菜单开始游戏，支持新游戏、读档、设置和返回主菜单
- 角色选择：新游戏会先选择矿工或勘探员，不同角色拥有不同生命、矿晶和初始道具
- 道具系统：支持探测灯预览隐藏矿格、绷带即时治疗，并在游戏内道具栏显示数量
- 操作优化：矿区格子支持右键标记，左键按当前模式翻开/标记/使用道具
- 结算界面：死亡或完成探索后显示本局层数、HP、矿晶、牌组和遗物摘要
- 怪物格战斗：扫雷矿区中翻出怪物会进入小型战斗，胜利后回到当前矿区继续探索
- 多语言：界面壳支持中文 / English 切换，并保存到用户设置
- 内容本地化：卡牌、敌人、事件、角色、道具、奖励等数据支持 `nameEn` / `descriptionEn` 等英文配置
- 存档：在路线选择等稳定节点自动保存跑局，主菜单可读档继续
- 配置驱动路线：矿井层级和房间选择来自 `data/layers.json`
- 战斗状态：卡牌和敌人意图支持 `damage` / `block` / `draw` / `weak` / `vulnerable`
- 战斗 UI：玩家、敌人、资源和意图分区显示，路线战斗胜利给卡牌奖励，矿区怪物战斗胜利给矿晶并返回棋盘
- 事件房间：支持多选项，并能造成伤害、治疗、增加生命上限、获得矿晶或加入卡牌
- 战斗机制：生命、格挡、能量、抽牌堆、弃牌堆、手牌
- 战斗奖励：胜利后获得矿晶、治疗，并可从奖励卡牌中选择一张加入牌组
- 商店与休整：可花费矿晶购买卡牌/治疗，休整点可治疗或换取矿晶与遗物记录
- 行动类型：`damage` / `block` / `draw`
- 基础 UI：跑局状态、房间描述、事件选项、敌人意图、手牌按钮、奖励选择、日志
=======
=======
>>>>>>> theirs
=======
>>>>>>> theirs
=======
>>>>>>> theirs
=======
>>>>>>> theirs
- 数据驱动：卡牌 / 敌人 / 初始牌组来源于 JSON 配置
- 战斗机制：生命、格挡、能量、抽牌堆、弃牌堆、手牌
- 进阶机制：`弱化(Weak)`、`易伤(Vulnerable)`、回复、临时能量
- 行动类型：`damage` / `block` / `draw` / `heal` / `energy` / `apply_weak` / `apply_vulnerable`
- 局内推进：连续遭遇战（样章 2 场）+ 胜利后三选一卡牌奖励（简版构筑成长）
- 遗物系统：开局随机获得 1 个遗物（能量/格挡/攻击加成）并影响整局战斗
- 事件节点：在关卡之间可进入营地回复或祭坛重铸遗物，且生命值在遭遇间持久化
- 分支路线：祭坛分支会切换为精英遭遇（更高难度）并提供更多奖励选项
- 地图预览：顶部显示当前样章路线（起点/岔路/普通线/精英线）
- 地图交互：事件阶段可直接点击顶部路线节点选择“营地线”或“精英线”
- 路线高亮：节点会按“已完成/当前/可选/锁定”状态显示不同高亮色
- 路线动效：当前节点带有轻微脉冲动画，并在节点间显示方向连线
- 悬停提示：鼠标悬停在分支节点会显示该路线的风险/收益说明
- 改进 UI：顶部信息面板 + 左侧战斗操作区 + 右侧日志区，配色偏深色矿井风格
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
>>>>>>> theirs
=======
>>>>>>> theirs
=======
>>>>>>> theirs
=======
>>>>>>> theirs
=======
>>>>>>> theirs

## 目录结构

- `x-game/project.godot`：Godot 项目入口（主场景 + 分辨率）
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
- `x-game/scenes/MainMenu.tscn`：主菜单、读档和设置入口
- `x-game/scenes/CharacterSelect.tscn`：角色选择场景
- `x-game/scenes/BattleScene.tscn`：探索 + 战斗一体化场景 UI
- `x-game/scripts/MainMenu.cs`：主菜单交互
- `x-game/scripts/CharacterSelect.cs`：角色选择与开局
- `x-game/scripts/Localization.cs`：中英文界面文本与语言设置
- `x-game/scripts/SaveManager.cs`：跑局存档读写
- `x-game/scripts/GameData.cs`：JSON 读取与数据索引
- `x-game/scripts/RunEngine.cs`：跑局状态、扫雷矿区、分层矿井路线、商店、休整、事件与奖励结算
=======
- `x-game/scenes/BattleScene.tscn`：战斗场景 UI
- `x-game/scripts/GameData.cs`：JSON 读取与数据索引
>>>>>>> theirs
=======
- `x-game/scenes/BattleScene.tscn`：战斗场景 UI
- `x-game/scripts/GameData.cs`：JSON 读取与数据索引
>>>>>>> theirs
=======
- `x-game/scenes/BattleScene.tscn`：战斗场景 UI
- `x-game/scripts/GameData.cs`：JSON 读取与数据索引
>>>>>>> theirs
=======
- `x-game/scenes/BattleScene.tscn`：战斗场景 UI
- `x-game/scripts/GameData.cs`：JSON 读取与数据索引
>>>>>>> theirs
=======
- `x-game/scenes/BattleScene.tscn`：战斗场景 UI
- `x-game/scripts/GameData.cs`：JSON 读取与数据索引
>>>>>>> theirs
- `x-game/scripts/BattleEngine.cs`：战斗状态机与规则
- `x-game/scripts/BattleScene.cs`：UI 渲染与输入处理
- `x-game/data/cards.json`：卡牌定义
- `x-game/data/enemies.json`：敌人定义（含意图循环）
- `x-game/data/decks.json`：初始牌组
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
- `x-game/data/events.json`：矿井事件与选项
- `x-game/data/rewards.json`：战斗奖励池
- `x-game/data/characters.json`：角色定义
- `x-game/data/items.json`：道具定义
- `x-game/data/layers.json`：矿井层级、房间类型和房间引用配置
=======
- `x-game/data/relics.json`：遗物定义
>>>>>>> theirs
=======
- `x-game/data/relics.json`：遗物定义
>>>>>>> theirs
=======
- `x-game/data/relics.json`：遗物定义
>>>>>>> theirs
=======
- `x-game/data/relics.json`：遗物定义
>>>>>>> theirs
=======
- `x-game/data/relics.json`：遗物定义
>>>>>>> theirs

## 运行方式

1. 使用 Godot 4.x（Mono / .NET）打开 `x-game/project.godot`。
2. 确保本地安装 .NET SDK。
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
3. 运行项目，进入主菜单。

## 后续建议

1. 将当前固定分层路线扩展为随机地图生成与事件池抽样。
2. 增加遗物的实际扫雷/战斗效果、局外成长和更完整的存档。
3. 增加 Debug 面板显示 seed、层数、棋盘统计和玩家状态。
4. 继续优化美术资源、动画反馈和完整双语内容配置。
=======
3. 运行项目，进入战斗场景。

## 显示稳定性说明

- 已针对高分辨率屏幕（如 2560x1440）调整布局：日志区不再使用 `fit_content` 自适应高度，改为稳定分栏布局，避免刷新日志时出现抖动/闪烁。
- 项目窗口拉伸模式为 `viewport`，并显式开启 VSync，减少高分辨率下的画面撕裂和闪烁概率。
- 不再强制 Windows 使用 `d3d12` 渲染驱动，避免不同显卡/驱动组合下出现兼容性异常。

## 下一步建议

1. 增加“遗物/装备”系统，形成局外构筑策略。
2. 增加多敌人战斗与目标选择。
3. 增加战斗外事件与地图流程。
>>>>>>> theirs
=======
3. 运行项目，进入战斗场景。

## 显示稳定性说明

- 已针对高分辨率屏幕（如 2560x1440）调整布局：日志区不再使用 `fit_content` 自适应高度，改为稳定分栏布局，避免刷新日志时出现抖动/闪烁。
- 项目窗口拉伸模式为 `viewport`，并显式开启 VSync，减少高分辨率下的画面撕裂和闪烁概率。
- 不再强制 Windows 使用 `d3d12` 渲染驱动，避免不同显卡/驱动组合下出现兼容性异常。

## 下一步建议

1. 增加“遗物/装备”系统，形成局外构筑策略。
2. 增加多敌人战斗与目标选择。
3. 增加战斗外事件与地图流程。
>>>>>>> theirs
=======
3. 运行项目，进入战斗场景。

## 显示稳定性说明

- 已针对高分辨率屏幕（如 2560x1440）调整布局：日志区不再使用 `fit_content` 自适应高度，改为稳定分栏布局，避免刷新日志时出现抖动/闪烁。
- 项目窗口拉伸模式为 `viewport`，并显式开启 VSync，减少高分辨率下的画面撕裂和闪烁概率。
- 不再强制 Windows 使用 `d3d12` 渲染驱动，避免不同显卡/驱动组合下出现兼容性异常。

## 下一步建议

1. 增加“遗物/装备”系统，形成局外构筑策略。
2. 增加多敌人战斗与目标选择。
3. 增加战斗外事件与地图流程。
>>>>>>> theirs
=======
3. 运行项目，进入战斗场景。

## 显示稳定性说明

- 已针对高分辨率屏幕（如 2560x1440）调整布局：日志区不再使用 `fit_content` 自适应高度，改为稳定分栏布局，避免刷新日志时出现抖动/闪烁。
- 项目窗口拉伸模式为 `viewport`，并显式开启 VSync，减少高分辨率下的画面撕裂和闪烁概率。
- 不再强制 Windows 使用 `d3d12` 渲染驱动，避免不同显卡/驱动组合下出现兼容性异常。

## 下一步建议

1. 增加“遗物/装备”系统，形成局外构筑策略。
2. 增加多敌人战斗与目标选择。
3. 增加战斗外事件与地图流程。
>>>>>>> theirs
=======
3. 运行项目，进入战斗场景。

## 显示稳定性说明

- 已针对高分辨率屏幕（如 2560x1440）调整布局：日志区不再使用 `fit_content` 自适应高度，改为稳定分栏布局，避免刷新日志时出现抖动/闪烁。
- 项目窗口拉伸模式为 `viewport`，并显式开启 VSync，减少高分辨率下的画面撕裂和闪烁概率。
- 不再强制 Windows 使用 `d3d12` 渲染驱动，避免不同显卡/驱动组合下出现兼容性异常。

## 下一步建议

1. 增加“遗物/装备”系统，形成局外构筑策略。
2. 增加多敌人战斗与目标选择。
3. 增加战斗外事件与地图流程。
>>>>>>> theirs
