...
pilot.astra.body|Balanced straight shots. Skill fires a piercing beam in your aim direction.|均衡直线射击。技能沿瞄准方向打出穿透光束。|Ровные прямые выстрелы. Навык бьет лучом по направлению прицела.|Tiros retos equilibrados. A habilidade dispara feixe perfurante na mira.|Ausgewogene Geradeaus-Schüsse. Der Skill feuert einen Piercing-Strahl in Zielrichtung.|Dengeli düz atış. Yetenek nişan yönüne delici ışın atar.|Tirs droits équilibrés. La compétence tire un rayon perforant vers la visée.|安定した直線射撃。スキルは照準方向へ貫通ビームを撃つ。
...
tactical.astra.tip|Piercing beam along your aim direction.|沿瞄准方向打出穿透光束。|Пробивающий луч по направлению прицела.|Feixe perfurante na direção da mira.|Piercing-Strahl in Zielrichtung.|Nişan yönüne delici ışın.|Rayon perforant vers la visée.|照準方向へ貫通ビーム。
tactical.vesper.tip|Long damage line in your aim direction.|沿瞄准方向生成长伤害线。|Длинная линия урона по прицелу.|Linha longa de dano na mira.|Lange Schadenslinie in Zielrichtung.|Nişan yönünde uzun hasar çizgisi.|Longue ligne de dégâts vers la visée.|照準方向に長いダメージ線。
...
upgrade.prism.body|Each shot fires one extra bullet.|每次射击多一发子弹。|Каждый выстрел дает еще одну пулю.|Cada tiro dispara uma bala extra.|Jeder Schuss feuert ein Extra-Projektil.|Her atış bir ekstra mermi atar.|Chaque tir lance un projectile en plus.|射撃ごとに弾を1発追加。
...
upgrade.leech.body|Kills can drop repair pickups.|击杀有概率掉落修复。|Убийства могут дать ремонт.|Abates podem soltar reparo.|Kills können Reparatur fallen lassen.|Öldürmeler onarım düşürebilir.|Les kills peuvent lâcher des réparations.|撃破で修理が出ることがある。
...
upgrade.rift.body|Shots become faster piercing bullets.|子弹变成更快的穿透弹。|Выстрелы становятся быстрыми пробивающими пулями.|Tiros viram balas perfurantes mais rápidas.|Schüsse werden schnellere Piercing-Projektile.|Atışlar hızlı delici mermi olur.|Les tirs deviennent plus rapides et perforants.|弾が高速貫通弾になる。
...
upgrade.storm.body|Pilot skill cooldown is shorter and fires extra shots.|角色技能冷却缩短，并追加反击弹。|Откат навыка меньше, он стреляет дополнительно.|Habilidade recarrega mais rápido e dispara tiros extras.|Skill lädt schneller und feuert Extraschüsse.|Yetenek beklemesi kısalır ve ekstra atış yapar.|La compétence recharge plus vite et tire en plus.|スキルCD短縮、追加弾を撃つ。
...
upgrade.echo.body|Weapon shots can fire an extra piercing shot.|武器射击有概率追加穿透弹。|Выстрелы иногда дают доп. пробивной снаряд.|Tiros podem disparar um perfurante extra.|Waffenschüsse können extra Piercing-Schuss feuern.|Silah atışı ekstra delici atış yapabilir.|Les tirs peuvent ajouter un tir perforant.|武器射撃が追加貫通弾を撃つことがある。
...
upgrade.solar.body|Pilot skills deal more damage.|角色技能伤害更高。|Навыки наносят больше урона.|Habilidades causam mais dano.|Skills verursachen mehr Schaden.|Yetenekler daha çok hasar verir.|Les compétences infligent plus de dégâts.|スキルダメージ増加。
...
upgrade.bounty.body|Next wave has more enemies and better drops.|下一波敌人更多，掉落更好。|Следующая волна: больше врагов и лучше дроп.|Próxima onda tem mais inimigos e drops melhores.|Nächste Welle hat mehr Feinde und bessere Drops.|Sonraki dalga daha kalabalık ve ganimet iyi.|Prochaine vague plus dense avec meilleurs butins.|次Waveは敵増加、ドロップ強化。
...
capstone.stasis|MAX RANK: once per run, clear all red bullets on screen.|满级：每局一次，清除屏幕内所有红弹。|МАКС: раз за забег чистит все красные пули на экране.|MÁX: uma vez por run, limpa todos tiros vermelhos.|MAX: einmal pro Run alle roten Kugeln räumen.|MAKS: koşu başına bir kez ekrandaki kırmızı mermiyi temizler.|MAX : une fois par run, nettoie tous les tirs rouges.|最大：1回の出撃で一度、画面内の赤弾を全消去。
