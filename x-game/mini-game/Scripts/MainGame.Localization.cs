...
language.hint|ENGLISH|中文|РУССКИЙ|PORTUGUÊS BR|DEUTSCH|TÜRKÇE|FRANÇAIS|日本語
...
menu.tip|Pick a pilot and difficulty, then launch. Spend Star Dust in the Fighter Bay after each run.|选择战机和难度后开始游戏。每局结束后，用星尘在战机升级室改装。|Выбери пилота и сложность, затем лети. Пыль трать в ангаре после вылета.|Escolha piloto e dificuldade, depois lance. Use Pó Estelar no hangar após a run.|Wähle Pilot und Stufe, dann starte. Sternenstaub nutzt du danach im Hangar.|Pilot ve zorluk seç, sonra kalk. Her seferden sonra Tozu hangarda harca.|Choisis pilote et difficulté, puis lance. Dépense la poussière au hangar après.|パイロットと難易度を選んで出撃。終了後は格納庫で星屑を使う。
...
title.start|START|开始游戏|СТАРТ|INICIAR|START|BAŞLA|COMMENCER|開始
tutorial.title|TRAINING RUN|新手训练|ТРЕНИРОВКА|TREINO|TRAINING|EĞİTİM|ENTRAÎNEMENT|チュートリアル
tutorial.start|Training run: Astra online.|训练开始：星棱已启动。|Учебный вылет: Астра на связи.|Treino: Astra online.|Trainingsflug: Astra bereit.|Eğitim: Astra hazır.|Entraînement : Astra en ligne.|訓練開始：アストラ起動。
tutorial.move.title|Move|移动|Движение|Movimento|Bewegung|Hareket|Déplacement|移動
tutorial.move.body|Move with keys or left stick. Keep moving to stay alive.|用键盘或左摇杆移动。保持移动，才能活下来。|Двигайся клавишами или левым стиком. Не стой на месте.|Mova com teclas ou analógico esquerdo. Continue se mexendo.|Bewege dich mit Tasten oder linkem Stick. Bleib in Bewegung.|Tuşlarla veya sol çubukla hareket et. Hayatta kalmak için durma.|Bouge au clavier ou au stick gauche. Reste mobile pour survivre.|キーまたは左スティックで移動。止まらず生き残ろう。
tutorial.lock.title|Auto Lock-On|自动锁敌|Автозахват|Trava automática|Auto-Zielerfassung|Oto kilit|Verrouillage auto|自動ロックオン
tutorial.lock.body|Weapons fire by themselves and lock nearby enemies. Focus on dodging red bullets.|武器会自动开火并锁定附近敌人。你专心躲红弹。|Оружие стреляет само и берет врагов рядом. Уклоняйся от красных пуль.|Armas atiram sozinhas e travam inimigos perto. Foque em desviar.|Waffen feuern selbst und erfassen nahe Gegner. Weiche roten Kugeln aus.|Silahlar otomatik ateş eder ve yakını kilitler. Kırmızı mermilerden kaç.|Les armes tirent seules et verrouillent les ennemis proches. Esquive le rouge.|武器は自動射撃し近くの敵をロック。赤弾回避に集中。
tutorial.dash.title|Dash Clears Bullets|冲刺清弹|Рывок чистит пули|Dash limpa tiros|Sprint räumt Kugeln|Atılma mermi temizler|Dash nettoie les tirs|ダッシュで弾消し
tutorial.dash.body|Dash toward your aim direction. The landing point clears a small area of red bullets.|向瞄准方向冲刺。落点会清除一小片红弹。|Рывок идет в сторону прицела. В точке выхода чистит красные пули.|Dê dash na direção da mira. Ao parar, limpa uma pequena área.|Sprinte in Zielrichtung. Am Ziel wird ein kleiner Bereich geräumt.|Nişan yönüne atıl. Varış noktasında küçük alan temizlenir.|Dash vers la visée. À l’arrivée, une petite zone de tirs est nettoyée.|狙った方向へダッシュ。到達点の赤弾を少し消す。
tutorial.ultimate.title|Emergency Clear|紧急清弹|Экстренная очистка|Limpeza de emergência|Noträumung|Acil temizleme|Nettoyage urgent|緊急弾消し
tutorial.ultimate.body|When energy is ready, clear nearby bullets to buy space.|能量足够时，清除身边红弹，抢回安全空间。|Когда энергия готова, очисти пули рядом и создай место.|Com energia pronta, limpe tiros próximos e ganhe espaço.|Ist Energie bereit, räume nahe Kugeln und schaffe Platz.|Enerji hazırsa yakındaki mermileri temizle ve alan aç.|Avec assez d’énergie, nettoie les tirs proches pour respirer.|エネルギーがあれば近くの弾を消し、安全を作る。
tutorial.flow.title|Run Flow|战斗流程|Ход вылета|Fluxo da run|Run-Ablauf|Sefer akışı|Déroulé du run|出撃の流れ
tutorial.flow.body|Collect gray EXP blocks. Fill the EXP bar to choose upgrades. After a run, spend Star Dust.|拾取灰色经验块。经验条满后选择升级。结束后用星尘成长。|Собирай серый опыт. Полная шкала дает выбор улучшения. После вылета трать пыль.|Colete blocos de EXP cinza. Barra cheia abre upgrades. Depois, gaste Pó Estelar.|Sammle graue EP. Volle Leiste öffnet Upgrades. Danach Sternenstaub ausgeben.|Gri EXP blokları topla. Bar dolunca geliştirme seç. Sonra Toz harca.|Ramasse l’EXP grise. Barre pleine : amélioration. Après, dépense la poussière.|灰色EXPを集める。満タンで強化選択。終了後は星屑を使う。
tutorial.hit|Training armor absorbed the hit.|训练护甲吸收了这次伤害。|Учебная броня поглотила удар.|Armadura de treino absorveu o golpe.|Trainingspanzerung hat den Treffer abgefangen.|Eğitim zırhı darbeyi emdi.|L’armure d’entraînement a absorbé le coup.|訓練装甲が被弾を吸収した。
...
upgrade.hint|Choose a card. Reroll only when none fit your build.|选择一张卡。三个都不适合当前流派时再重抽。|Выбери карту. Реролль, только если ничего не подходит.|Escolha uma carta. Role de novo só se nada servir.|Wähle eine Karte. Neu würfeln nur, wenn nichts passt.|Bir kart seç. Hiçbiri uymuyorsa yenile.|Choisis une carte. Relance seulement si rien ne convient.|カードを選ぶ。合わない時だけ再抽選。
...
end.meta_hint|Restart · Main Menu · Fighter Bay|重新开始 · 主界面 · 战机升级室|Повтор · Главное меню · Ангар|Reiniciar · Menu · Hangar|Neustart · Hauptmenü · Hangar|Tekrar · Ana menü · Hangar|Rejouer · Menu · Hangar|再出撃 · メニュー · 格納庫
...
settings.adjust_hint|Adjust values, then confirm.|调整数值后确认。|Измени значение и подтверди.|Ajuste o valor e confirme.|Wert ändern, dann bestätigen.|Değeri ayarla, sonra onayla.|Ajuste les valeurs, puis valide.|値を調整して決定。
