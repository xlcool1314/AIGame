...
public partial class BattleScene : Control
{
    ...
    private Button _infoToggleButton = null!;
    ...
    private VBoxContainer _battleLayout = null!;
    private HBoxContainer _combatantRow = null!;
    ...
    private const float RouteMapWidth = 1220f;
    private const float RouteNodeWidth = 122f;
    private const float RouteNodeHeight = 70f;
    private const float RouteLayerGap = 134f;
    private const float RouteMapSidePadding = 220f;
    private const float RouteMapTopPadding = 56f;
    private const float RouteMapBottomPadding = 90f;
    ...
    private readonly Dictionary<int, Control> _enemyTargetButtons = new();
    private Control? _aimingCardButton;
    private int _aimingCardIndex = -1;
    private int _aimHoverEnemyIndex = -1;
    ...
    private MiniGameData? _activeMiniGame;
    private readonly HashSet<int> _selectedMiniGameOptions = new();
    ...
    public override void _Ready()
    {
        ...
        _battleLayout = GetNode<VBoxContainer>("Root/Margin/MainLayout/ContentSplit/MainPanel/ContentStack/BattlePanel/BattleLayout");
        _combatantRow = GetNode<HBoxContainer>("Root/Margin/MainLayout/ContentSplit/MainPanel/ContentStack/BattlePanel/BattleLayout/CombatantRow");
        ...
        {
            GameSession.EnsureSelectedCharacter(_gameData);
            _run.StartRun(_gameData);
        }
    ...
    private void ShowDeckModal()
    {
        ...
        for (var i = 0; i < _run.PlayerDeck.Count; i++)
        {
            ...
            var footer = Localization.Language == Localization.English
                ? $"Deck #{i + 1}"
                : $"牌组 #{i + 1}";
            var cell = CardCell.Create(card, CardCellMode.Deck, true, footer, i);
            cell.TooltipText = BuildCardTooltip(card);
            cell.MouseDefaultCursorShape = CursorShape.Arrow;
    ...
    private void BuildHudBar()
    {
        ...
        _infoToggleButton = new Button
        {
            Name = "InfoToggleButton",
            CustomMinimumSize = new Vector2(104, 52),
            FocusMode = FocusModeEnum.None
        };
        _infoToggleButton.Pressed += ToggleInfoPanel;
        _topBarLayout.AddChild(_infoToggleButton);
    ...
    {
        SetInfoPanelExpanded(!_infoPanelExpanded);
    }

    private void SetInfoPanelExpanded(bool expanded)
    {
        _infoPanelExpanded = expanded;
        _rightPanel.Visible = expanded;
        _contentSplit.AddThemeConstantOverride("separation", expanded ? 12 : 8);
        UpdateInfoToggleButton();
    }

    private void UpdateInfoToggleButton()
    {
        if (_infoToggleButton == null)
        {
            return;
        }

        var english = Localization.Language == Localization.English;
        _infoToggleButton.Text = _infoPanelExpanded
            ? english ? "Hide Info" : "收起信息"
            : english ? "Info" : "信息";
        _infoToggleButton.TooltipText = _infoPanelExpanded
            ? english ? "Collapse room information and log." : "收起房间信息与记录。"
            : english ? "Show room information and log." : "打开房间信息与记录。";
    }
    ...
    private void BuildEnemyTargetRow()
    {
        ...
        _enemyCombatLabel.Visible = false;
        ...
        _enemyTargetRow.AddThemeConstantOverride("separation", 14);
        _enemyCombatLayout.AddChild(_enemyTargetRow);
    }

    private void BuildBattleInteractionLayout()
    {
        _battlePanel.SizeFlagsVertical = SizeFlags.ExpandFill;
        _battlePanel.CustomMinimumSize = new Vector2(0, 660);
        _battleLayout.AddThemeConstantOverride("separation", 6);

        _combatantRow.CustomMinimumSize = new Vector2(0, 318);
        _combatantRow.SizeFlagsVertical = SizeFlags.ShrinkBegin;
        _combatantRow.AddThemeConstantOverride("separation", 14);

        _playerCombatPanel.CustomMinimumSize = new Vector2(286, 302);
        _playerCombatPanel.SizeFlagsHorizontal = SizeFlags.ShrinkBegin;
        _enemyCombatPanel.CustomMinimumSize = new Vector2(0, 302);
        _enemyCombatPanel.SizeFlagsHorizontal = SizeFlags.ExpandFill;
        _enemyCombatLayout.AddThemeConstantOverride("separation", 8);
        _enemyTargetRow.CustomMinimumSize = new Vector2(0, 270);

        _playerPortrait.CustomMinimumSize = new Vector2(0, 144);
        _playerHpBar.CustomMinimumSize = new Vector2(0, 16);
        _enemyHpBar.CustomMinimumSize = new Vector2(0, 16);

        _battleResourceLabel.AddThemeFontSizeOverride("font_size", 17);
        _battleResourceLabel.HorizontalAlignment = HorizontalAlignment.Center;
        _statusHelpLabel.AddThemeFontSizeOverride("font_size", 14);
        _statusHelpPanel.CustomMinimumSize = new Vector2(0, 44);
        _statusHelpPanel.Visible = false;

        _handPanel.CustomMinimumSize = new Vector2(0, 252);
        _handPanel.SizeFlagsVertical = SizeFlags.ShrinkBegin;
        _handBox.CustomMinimumSize = new Vector2(0, 244);
        _endTurnButton.CustomMinimumSize = new Vector2(0, 58);
    ...
    private void EnterNextRoom(int choiceIndex)
    {
        ...
        switch (room.Kind)
        {
            ...
            case "calibration":
                ...
                break;
            case "minigame":
                RenderMiniGameRoom(room);
                break;
            case "complete":
                ShowEndPanel(true, "你在天亮前完成了床下冒险。");
                break;
        }
    ...
    private static string GetRoomIcon(string kind)
    {
        if (kind == "minigame")
        {
            return Localization.Language == Localization.English ? "[Puzzle]" : "[小游戏]";
        }
    ...
    {
        if (room.Kind == "minigame")
        {
            return FormatMiniGameSummary(room);
        }

        return room.Kind switch
        {
            "battle" => $"{string.Format(Localization.T("encounter_reward"), GetPrimaryRoomEnemy(room).DisplayName())} {FormatEnemyTestType(GetPrimaryRoomEnemy(room))} {FormatThreatPreview(room)}",
            "elite" => $"{(Localization.Language == Localization.English ? "Elite battle. Higher pressure, stronger spoils." : "精英战。压力更高，但战利品更强。")} {FormatEnemyTestType(GetPrimaryRoomEnemy(room))} {FormatThreatPreview(room)}",
            "treasure" => Localization.Language == Localization.English ? "Choose a cache reward: keepsake, card, or supplies." : "选择一份宝藏补给：纪念物、卡牌或工具。",
            ...
            "calibration" => Localization.Language == Localization.English
                ? "Tinker with a house gadget. Choose 3 nodes for buttons, battery, quiet, cards, and risk."
                : "摆弄小屋机关。选择 3 个节点，组合纽扣、手电电量、安静值、卡牌与失控风险。",
            "complete" => Localization.T("complete_summary"),
            _ => Localization.T("unknown_room")
        };
    }

    private EnemyData GetPrimaryRoomEnemy(RunRoom room)
    {
        if (!string.IsNullOrWhiteSpace(room.EnemyId))
        {
            return _gameData.GetEnemy(room.EnemyId);
        }

        if (room.EnemyIds.Count > 0)
        {
            return _gameData.GetEnemy(room.EnemyIds[0]);
        }

        return _gameData.GetEnemy("slime");
    }

    private static string FormatEnemyTestType(EnemyData enemy)
    {
        var key = !string.IsNullOrWhiteSpace(enemy.TestType)
            ? enemy.TestType
            : enemy.Tags.Count > 0 ? enemy.Tags[0] : string.Empty;
        ...
        {
            "baseline_check" => Localization.Language == Localization.English ? "baseline tactics" : "基础攻防",
            "defense_check" => Localization.Language == Localization.English ? "defense check" : "防御检验",
            "burst_check" => Localization.Language == Localization.English ? "burst check" : "爆发检验",
            "shield_check" => Localization.Language == Localization.English ? "shield break" : "破盾检验",
            "sustain_check" => Localization.Language == Localization.English ? "sustain check" : "续航检验",
            "control_check" => Localization.Language == Localization.English ? "control check" : "控制检验",
            "scaling_check" => Localization.Language == Localization.English ? "scaling check" : "成长检验",
            "boss_pressure" => Localization.Language == Localization.English ? "boss pressure" : "首领压力",
            _ => Localization.Language == Localization.English ? key.Replace('_', ' ') : key
        };

        return Localization.Language == Localization.English ? $"Test: {label}" : $"考验：{label}";
    }
    ...
    private void AddRouteLayerBands(Control mapCanvas)
    {
        for (var layerIndex = 0; layerIndex < _run.MapLayers.Count; layerIndex++)
        {
            ...
            var band = new ColorRect
            {
                ...
                Position = new Vector2(96, y - 16),
                Size = new Vector2(RouteMapWidth - 118, RouteNodeHeight + 32),
            ...
            var label = new Label
            {
                ...
                Position = new Vector2(8, y + 12),
                Size = new Vector2(82, 46),
        ...
        for (var layerIndex = 0; layerIndex < _run.MapLayers.Count - 1; layerIndex++)
        {
            foreach (var room in _run.MapLayers[layerIndex])
            {
                foreach (var nextNodeId in room.NextNodeIds)
                {
                    ...
                    var active = room.NodeId == _run.CurrentRoomNodeId && _run.IsRoomReachable(next);
                    AddRouteConnectionLine(mapCanvas, room, next, active);
                }
            }
        }
    }
    ...
    private void AddRouteNodes(Control mapCanvas, IReadOnlyList<RunRoom> choices, HashSet<string> availableRoute)
    {
        for (var layerIndex = 0; layerIndex < _run.MapLayers.Count; layerIndex++)
        {
            foreach (var room in _run.MapLayers[layerIndex])
            {
                ...
                var routeAnchor = room.NodeId == _run.CurrentRoomNodeId;
                var current = routeAnchor && !_run.HasNextLayer();
                var past = room.LayerIndex < _run.CurrentLayerIndex || routeAnchor || (room.LayerIndex == _run.CurrentLayerIndex && !routeAnchor);
                var onRoute = routeAnchor || selectable || availableRoute.Contains(room.NodeId);
    ...
    {
        var focusLayer = Math.Clamp(_run.CurrentLayerIndex < 0 ? 0 : _run.CurrentLayerIndex + 1, 0, Math.Max(0, _run.MapLayers.Count - 1));
        return Math.Max(0, (int)(GetRouteLayerY(focusLayer) - 220f));
    }
    ...
    {
        var title = Localization.Language == Localization.English
            ? layerIndex == 0 ? "Entry" : $"Room {layerIndex + 1}"
            : layerIndex == 0 ? "暗门" : $"第 {layerIndex + 1} 间";
        if (layerIndex <= _run.CurrentLayerIndex)
        {
            return $"{title}\n{Localization.T("route_visited")}";
        }
    ...
    {
        var marker = past && onRoute ? "✓" : current ? "●" : selectable ? "◆" : !onRoute ? "·" : "◇";
        var title = TrimRouteTitle(room.DisplayTitle(), Localization.Language == Localization.English ? 12 : 7);
    ...
    private static string FormatCompactRouteCost(RunRoom room)
    {
        ...
        return Localization.Language == Localization.English
            ...
            : $"险{room.Risk} 电-{room.LampCost}{bonus}";
    }
    ...
    private string FormatRouteNodeSummary(RunRoom room)
    {
        if (room.Kind == "minigame")
        {
            return FormatMiniGameRouteSummary(room);
        }
        ...
        return room.Kind switch
        {
            ...
            "treasure" => Localization.Language == Localization.English ? "Keepsake / card / supplies" : "纪念物 / 卡牌 / 补给",
            ...
            "calibration" => Localization.Language == Localization.English ? "Pick 3 gadget nodes" : "选择 3 个机关节点",
    ...
    private static string FormatRouteCost(RunRoom room)
    {
        var bonus = room.RewardBonus > 0 ? $" +{room.RewardBonus}%" : string.Empty;
        return Localization.Language == Localization.English
            ? $"R{room.Risk}  Battery -{room.LampCost}{bonus}"
            : $"险{room.Risk}  手电 -{room.LampCost}{bonus}";
    }
    ...
    private static string GetMapNodeIcon(string kind)
    {
        if (kind == "minigame")
        {
            return Localization.Language == Localization.English ? "P" : "戏";
        }
    ...
    private static string BuildRoomLoadingText(RunRoom room)
    {
        if (room.Kind == "minigame")
        {
            return Localization.Language == Localization.English ? "Reading the mechanism..." : "检查机关装置……";
        }
        ...
        if (Localization.Language == Localization.English)
        {
            return room.Kind switch
            {
                "mine" => "Opening the room grid...",
                "battle" or "elite" => "Entering combat...",
                "treasure" => "Opening the cache...",
                "event" => "Reading the tunnel signs...",
                "rest" => "Securing a quiet stop...",
                "shop" => "Finding the trade corner...",
                "calibration" => "Checking the house gadget...",
                "minigame" => "Reading the mechanism...",
    ...
    private string FormatThreatPreview(RunRoom room)
    {
        ...
        return Localization.Language == Localization.English
            ? $"Threat {threat}. Noise cards: {fogCards}."
            : $"威胁 {threat}。吵闹卡 {fogCards}。";
    }
    ...
    private static string FormatRoomRisk(RunRoom room)
    {
        ...
        return $"[{risk} | 手电-{room.LampCost}{bonus}]";
    }

    private static string FormatMineSummary(MineRoomConfig config)
    {
        var partialReward = config.PartialReward > 0 ? config.PartialReward : Math.Max(4, Math.Max(0, config.ClearReward) * 45 / 100);
        var partialPercent = Math.Clamp(config.PartialRewardPercent, 30, 90);
        if (Localization.Language == Localization.English)
        {
            return $"{config.Width}x{config.Height} board. Threats: {config.Monsters + config.Traps}; finds: {config.Treasures + config.Ores}; stage {partialPercent}% +{partialReward}; clear +{config.ClearReward}.";
        }

        return $"{config.Width}x{config.Height} 探索棋盘。危险：{config.Monsters + config.Traps}；藏品：{config.Treasures + config.Ores}；探索 {partialPercent}% +{partialReward}；全清 +{config.ClearReward}。";
    }
    ...
    private static string FormatMineIntro(MineRoomConfig config)
    {
        ...
        if (Localization.Language == Localization.English)
        {
            return $"Search the room. Numbers hint at nearby danger and finds. Exit can end the room; {partialPercent}% safe reveal grants +{partialReward}, full clear grants +{config.ClearReward}.";
        }

        return $"房间探索开始。数字提示周围危险与藏品。找到出口即可离开；安全格探索 {partialPercent}% 获得 +{partialReward}，全清额外 +{config.ClearReward}。";
    }
    ...
    private void StartMineMonsterBattle()
    {
        ...
        _roomDescriptionLabel.Text = $"房间格子里的怪物拦住了去路。威胁等级 {_battle.ThreatLevel}，击退它后可以回到当前房间继续探索。";
        RenderBattle();
    }
    ...
    private string BuildMineRevealTipText(MineRevealResult result, int index, int beforeHp, int beforeShards, int beforeFog)
    {
        var title = result switch
        {
            ...
            MineRevealResult.Ore => Localization.Language == Localization.English ? "Button Found" : "找到纽扣",
            MineRevealResult.Exit => Localization.Language == Localization.English ? "Exit Found" : "发现出口",
            MineRevealResult.Cleared => Localization.Language == Localization.English ? "Room Cleared" : "房间清理完成",
            _ => Localization.Language == Localization.English ? "Tile Revealed" : "翻开房间格子"
        };
        ...
        if (cell != null && result is MineRevealResult.Revealed or MineRevealResult.Cleared)
        {
            details.Add(Localization.Language == Localization.English
                ? $"Clues: danger {cell.DangerClue}, finds {cell.RewardClue}."
                : $"线索：危险 {cell.DangerClue}，藏品 {cell.RewardClue}。");
        }
        ...
        if (shardDelta != 0)
        {
            details.Add(Localization.Language == Localization.English ? $"Buttons {FormatSigned(shardDelta)}" : $"纽扣 {FormatSigned(shardDelta)}");
        }
        if (fogDelta != 0)
        {
            details.Add(Localization.Language == Localization.English ? $"Noise {FormatSigned(fogDelta)}" : $"声响 {FormatSigned(fogDelta)}");
        }
    ...
    private void RenderMinefield()
    {
        ...
        var stageState = minefield.PartialRewardClaimed ? "✓" : $"{minefield.PartialRewardPercent}%";
        var clearState = minefield.PerfectRewardClaimed ? "✓" : "100%";
        _mineStatusLabel.Text = Localization.Language == Localization.English
            ? $"Safe {minefield.SafeRevealCount}/{minefield.SafeRevealGoal} ({minefield.SafeRevealPercent}%)  Flags {flags}/{minefield.DangerCount}  Trap -{minefield.TrapDamage} HP  Stage {stageState} +{minefield.PartialRewardShards}  Clear {clearState} +{minefield.RewardShards}"
            : $"安全 {minefield.SafeRevealCount}/{minefield.SafeRevealGoal}（{minefield.SafeRevealPercent}%）  标记 {flags}/{minefield.DangerCount}  陷阱 -{minefield.TrapDamage}  阶段 {stageState} +{minefield.PartialRewardShards}  全清 {clearState} +{minefield.RewardShards}";
    ...
    private void RenderTreasureRoom(RunRoom room)
    {
        ...
        {
            AddChoiceButton($"{(Localization.Language == Localization.English ? "Take keepsake" : "带走纪念物")}：{relic.DisplayName()}\n{relic.DisplayDescription()}", () => OnTreasureRelicPicked(relic));
        }
        ...
        {
            var cardGrid = CreateRewardCardGrid("TreasureCardGrid", Math.Min(2, Math.Max(1, cardChoices.Count)));
            foreach (var card in cardChoices)
            {
                AddTreasureCardChoice(cardGrid, card);
            }

            _choiceList.AddChild(cardGrid);
        }

        AddChoiceButton(Localization.Language == Localization.English
            ? "Take search supplies\nGain Probe Flashlight x2, Bandage x1, and reduce noise by 1."
            : "拿走探索补给\n获得探测手电 x2、绷带 x1，并降低 1 点声响。",
    ...
    private void AddTreasureCardChoice(Container parent, CardData card)
    {
        var footer = Localization.Language == Localization.English
            ? "Record this card"
            : "记录这张卡";
    ...
    {
        _roomTitleLabel.Text = room.DisplayTitle();
        _roomDescriptionLabel.Text = "手电在旧木箱旁轻轻晃着，你终于有片刻能把呼吸放慢。";
        _choicePanel.Visible = true;

        AddChoiceButton("休息\n恢复一大段勇气。", () => OnRestChoicePressed("heal"));
        AddChoiceButton(Localization.Language == Localization.English ? "Check flashlight\nRestore 25 battery and reduce noise by 3." : "检查手电\n恢复 25 手电电量，并降低 3 点声响。", () => OnRestChoicePressed("calibrate"));
        ...
        AddChoiceButton("整备\n获得 12 纽扣和纪念物：调好的手电。", () => OnRestChoicePressed("forge"));
    }
    ...
    private void OnRestChoicePressed(string mode)
    {
        ...
        ShowGameTip(mode switch
        {
            ...
            "calibrate" => Localization.Language == Localization.English ? "Flashlight checked." : "手电已检查。",
    ...
    {
        _roomTitleLabel.Text = room.DisplayTitle();
        _roomDescriptionLabel.Text = "一盏挂满纸星星的小灯在角落摇晃。地下朋友愿意收纽扣，也愿意卖一点生路。";
        _choicePanel.Visible = true;
        ...
        {
            AddChoiceButton($"{(Localization.Language == Localization.English ? "Buy keepsake" : "交换纪念物")} {relic.DisplayName()} - 34 纽扣\n{relic.DisplayDescription()}", () => OnBuyRelicPressed(relic));
        }

        AddChoiceButton("交换零食 - 16 纽扣\n恢复 18 点勇气。", OnBuyHealPressed);
        AddChoiceButton(Localization.Language == Localization.English ? "Buy Probe Flashlight x2 - 14 Buttons\nPreview hidden room tiles before committing." : "交换探测手电 x2 - 14 纽扣\n在翻开房间格子前先预览隐藏内容。", () => OnBuyItemPressed("probe_lamp", 2, 14));
        AddChoiceButton(Localization.Language == Localization.English ? "Buy Smoke Sticker - 12 Buttons\nPreview a tile; dangerous tiles are marked automatically." : "交换烟雾贴纸 - 12 纽扣\n预览 1 个房间格子；若为危险格会自动标记。", () => OnBuyItemPressed("smoke_marker", 1, 12));
        AddChoiceButton("升级一张牌 - 18 纽扣\n让已有卡牌变成强化版本。", () => RunLoadingTransition(
            Localization.Language == Localization.English ? "Opening the forge..." : "展开锻造台……",
            () => RenderDeckUpgradeChoices(18)));
        AddChoiceButton("移除一张牌 - 20 纽扣\n精简牌组，提高关键牌上手率。", () => RunLoadingTransition(
            Localization.Language == Localization.English ? "Sorting the deck..." : "整理牌组……",
            () => RenderDeckRemoveChoices(20)));
        AddChoiceButton("离开交换铺\n保留纽扣继续深入。", OnLeaveShopPressed);
    ...
    {
        _run.Log.Add("你离开地下交换铺，继续向下。");
        _choicePanel.Visible = false;
        _continueButton.Visible = true;
        RenderShared();
        ShowGameTip(Localization.Language == Localization.English ? "Left the trade corner." : "已离开交换铺。");
    }
    ...
    private void RenderCalibrationRoom(RunRoom room)
    {
        ...
        _roomTitleLabel.Text = room.DisplayTitle();
        _roomDescriptionLabel.Text = Localization.Language == Localization.English
            ? "A cracked house gadget still hums. Select exactly 3 nodes to route power through. Stable sets are safe; greedy sets can surge."
            : "一台裂纹小屋机关仍在低鸣。选择 3 个节点接入回路。稳态组合更安全，贪心组合可能过载。";
        _choicePanel.Visible = true;
    ...
    private void BuildCalibrationNodes(RunRoom room)
    {
        ...
        {
            new("stable_crystal", "稳定纸星", "Stable Paper Star", "稳态 / 安静", "Stable / Quiet", shards: 8, fogReduction: 1, tags: new[] { "stable", "fog" }),
            new("ore_prism", "纽扣棱镜", "Button Prism", "纽扣 / 微量声响", "Buttons / Minor noise", shards: 24, fogGain: 1, tags: new[] { "ore", "risk" }),
            new("oil_wick", "备用电池", "Spare Battery", "电量 / 稳态", "Battery / Stable", oil: 22, tags: new[] { "oil", "stable" }),
            new("survey_map", "测绘星图", "Survey Map", "卡牌 / 稳态", "Card / Stable", cardId: "mapping_flare", tags: new[] { "card", "stable" }),
            new("rift_resonator", "裂缝共鸣器", "Rift Resonator", "纽扣 / 过载", "Buttons / Surge", shards: 16, damage: 5, cardId: "crack_finder", tags: new[] { "ore", "risk", "card" }),
            new("mist_lens", "安静透镜", "Quiet Lens", "安静 / 电量", "Quiet / Battery", oil: 8, fogReduction: 2, tags: new[] { "fog", "oil" })
        };
    ...
    {
        _run.Log.Add(Localization.Language == Localization.English ? "You bypassed the gadget." : "你绕过了小屋机关。");
        _choicePanel.Visible = false;
        _continueButton.Visible = true;
        RenderShared();
        ShowGameTip(Localization.Language == Localization.English ? "Gadget bypassed." : "已绕过小屋机关。");
    }

    private void RenderMiniGameRoom(RunRoom room)
    {
        _activeMiniGame = _gameData.GetMiniGame(room.MiniGameId);
        _selectedMiniGameOptions.Clear();
        _roomTitleLabel.Text = _activeMiniGame.DisplayTitle();
        _roomDescriptionLabel.Text = _activeMiniGame.DisplayDescription();
        _choicePanel.Visible = true;
        RenderMiniGameChoices();
    }

    private void RenderMiniGameChoices()
    {
        ClearBox(_choiceList);
        ...
        var pickCount = Math.Clamp(_activeMiniGame.PickCount, 1, Math.Max(1, _activeMiniGame.Options.Count));
        var summary = new Label
        {
            Text = BuildMiniGamePreviewText(_activeMiniGame, pickCount),
            AutowrapMode = TextServer.AutowrapMode.WordSmart
        };
        summary.AddThemeColorOverride("font_color", Color.FromHtml("d8e2ee"));
        _choiceList.AddChild(summary);

        var grid = new GridContainer
        {
            Columns = _activeMiniGame.Type == "draft" ? 2 : 1,
            SizeFlagsHorizontal = SizeFlags.ExpandFill
        };
        ...
        for (var i = 0; i < _activeMiniGame.Options.Count; i++)
        {
            var option = _activeMiniGame.Options[i];
            var selected = _selectedMiniGameOptions.Contains(i);
            var button = new Button
            {
                Text = $"{(selected ? "◆ " : "◇ ")}{option.DisplayText()}\n{option.DisplayHint()}\n{FormatMiniGameActions(option.Actions)}",
                CustomMinimumSize = new Vector2(0, _activeMiniGame.Type == "draft" ? 112 : 78),
                SizeFlagsHorizontal = SizeFlags.ExpandFill,
                AutowrapMode = TextServer.AutowrapMode.WordSmart,
                Disabled = !selected && _selectedMiniGameOptions.Count >= pickCount
            };
            StyleButton(button, selected ? Color.FromHtml("5b4a2a") : Color.FromHtml("263445"), selected ? Color.FromHtml("fff1d0") : Color.FromHtml("d8e2ee"));
            var captured = i;
            button.Pressed += () => ToggleMiniGameOption(captured);
            grid.AddChild(button);
        }

        var confirm = new Button
        {
            Text = Localization.Language == Localization.English ? "Confirm" : "确认执行",
            CustomMinimumSize = new Vector2(0, 58),
            AutowrapMode = TextServer.AutowrapMode.WordSmart,
            Disabled = _selectedMiniGameOptions.Count != pickCount
        };
        StyleButton(confirm, Color.FromHtml("315f46"), Color.FromHtml("e7fff1"));
        confirm.Pressed += CompleteMiniGame;
        _choiceList.AddChild(confirm);

        AddChoiceButton(Localization.Language == Localization.English
            ? "Skip\nLeave the mechanism untouched."
            : "跳过\n不触碰这处机关。",
            BypassMiniGame);
    }

    private void ToggleMiniGameOption(int index)
    {
        if (_activeMiniGame == null)
        {
            return;
        ...
        if (_selectedMiniGameOptions.Contains(index))
        {
            _selectedMiniGameOptions.Remove(index);
        }
        else if (_selectedMiniGameOptions.Count < Math.Max(1, _activeMiniGame.PickCount))
        {
            _selectedMiniGameOptions.Add(index);
        }
    ...
    private void CompleteMiniGame()
    {
        ...
        foreach (var index in _selectedMiniGameOptions)
        {
            var option = _activeMiniGame.Options[index];
            actions.AddRange(option.Actions);
        ...
        foreach (var combo in _activeMiniGame.Combos)
        {
            if (tagCounts.GetValueOrDefault(combo.Tag) < Math.Max(1, combo.Count))
            {
                continue;
            }

            actions.AddRange(combo.Actions);
            if (!string.IsNullOrWhiteSpace(combo.DisplayResult()))
            {
                comboNotes.Add(combo.DisplayResult());
            }
        }

        var reason = _activeMiniGame.DisplayTitle();
        if (comboNotes.Count > 0)
        {
            reason = $"{reason}: {string.Join(" / ", comboNotes)}";
        }

        _run.ApplyMiniGameActions(actions, _gameData, reason);
        _roomDescriptionLabel.Text = comboNotes.Count > 0
            ? string.Join("\n", comboNotes)
            : (Localization.Language == Localization.English ? "The mechanism settles into a new rhythm." : "机关的震动慢慢稳定下来。");
        _choicePanel.Visible = false;
        _continueButton.Visible = true;
        RenderShared();
        ShowGameTip(Localization.Language == Localization.English ? "Mechanism resolved." : "机关已处理。");
        ShowRunDeltaTips(before);
    }

    private void BypassMiniGame()
    {
        _run.Log.Add(Localization.Language == Localization.English ? "You bypassed the mechanism." : "你绕过了机关。");
        _choicePanel.Visible = false;
        _continueButton.Visible = true;
        RenderShared();
        ShowGameTip(Localization.Language == Localization.English ? "Mechanism skipped." : "已绕过机关。");
    }

    private string BuildMiniGamePreviewText(MiniGameData miniGame, int pickCount)
    {
        var text = Localization.Language == Localization.English
            ? $"Selected {_selectedMiniGameOptions.Count}/{pickCount}. These route mechanisms appear only in some runs."
            : $"已选择 {_selectedMiniGameOptions.Count}/{pickCount}。路线机关只会在部分局中出现。";
        if (_selectedMiniGameOptions.Count == 0)
        {
            return text;
        }
        ...
        foreach (var index in _selectedMiniGameOptions)
        {
            actions.AddRange(miniGame.Options[index].Actions);
        }

        return $"{text}\n{FormatMiniGameActions(actions)}";
    }

    private string FormatMiniGameSummary(RunRoom room)
    {
        if (string.IsNullOrWhiteSpace(room.MiniGameId))
        {
            return Localization.Language == Localization.English ? "Special mechanism." : "特殊机关。";
        }

        return _gameData.GetMiniGame(room.MiniGameId).DisplayDescription();
    }

    private string FormatMiniGameRouteSummary(RunRoom room)
    {
        if (string.IsNullOrWhiteSpace(room.MiniGameId))
        {
            return Localization.Language == Localization.English ? "Special mechanism" : "特殊机关";
        ...
        var miniGame = _gameData.GetMiniGame(room.MiniGameId);
        return Localization.Language == Localization.English
            ? $"{miniGame.DisplayTitle()} / pick {Math.Max(1, miniGame.PickCount)}"
            : $"{miniGame.DisplayTitle()} / 选 {Math.Max(1, miniGame.PickCount)}";
    }

    private string FormatMiniGameActions(IEnumerable<RunAction> actions)
    {
        ...
        foreach (var action in actions)
        {
            ...
            {
                case "heal":
                    parts.Add(Localization.Language == Localization.English ? $"Courage +{action.Value}" : $"勇气 +{action.Value}");
                    break;
                case "damage":
                    parts.Add(Localization.Language == Localization.English ? $"Courage -{action.Value}" : $"勇气 -{action.Value}");
                    break;
                case "max_hp":
                    parts.Add(Localization.Language == Localization.English ? $"Max courage +{action.Value}" : $"勇气上限 +{action.Value}");
                    break;
                case "shards":
                    parts.Add(Localization.Language == Localization.English ? $"Buttons +{action.Value}" : $"纽扣 +{action.Value}");
                    break;
                case "lamp":
                    parts.Add(Localization.Language == Localization.English ? $"Battery {FormatSigned(action.Value)}" : $"手电 {FormatSigned(action.Value)}");
                    break;
                case "fog":
                    parts.Add(Localization.Language == Localization.English ? $"Noise {FormatSigned(action.Value)}" : $"声响 {FormatSigned(action.Value)}");
                    break;
                case "add_card":
                    parts.Add(Localization.Language == Localization.English ? $"Card: {_gameData.GetCard(action.CardId).DisplayName()}" : $"卡牌：{_gameData.GetCard(action.CardId).DisplayName()}");
                    break;
                case "relic":
                    parts.Add(Localization.Language == Localization.English ? $"Keepsake: {_gameData.GetRelic(action.RelicId).DisplayName()}" : $"纪念物：{_gameData.GetRelic(action.RelicId).DisplayName()}");
                    break;
                case "item":
                    parts.Add($"{_gameData.GetItem(action.ItemId).DisplayName()} x{Math.Max(1, action.Value)}");
                    break;
            }
        }

        return parts.Count == 0
            ? (Localization.Language == Localization.English ? "No immediate effect." : "无直接效果。")
            : string.Join("  |  ", parts);
    }
    ...
    private CalibrationOutcome CalculateCalibrationOutcome()
    {
        ...
        if (tagCounts.GetValueOrDefault("stable") >= 2)
        {
            ...
            outcome.Notes.Add(Localization.Language == Localization.English ? "Stable circuit: battery +8, noise -1." : "稳态回路：手电 +8，声响 -1。");
        }
        if (tagCounts.GetValueOrDefault("ore") >= 2)
        {
            outcome.Shards += 16;
            outcome.Notes.Add(Localization.Language == Localization.English ? "Button resonance: buttons +16." : "纽扣共振：纽扣 +16。");
        }
        if (tagCounts.GetValueOrDefault("oil") >= 2)
        {
            outcome.Oil += 12;
            outcome.Notes.Add(Localization.Language == Localization.English ? "Battery loop: battery +12." : "电量回路：手电 +12。");
        }
        if (tagCounts.GetValueOrDefault("fog") >= 2)
        {
            outcome.FogReduction += 1;
            outcome.Notes.Add(Localization.Language == Localization.English ? "Quiet lock: noise -1." : "安静锁定：声响 -1。");
        }
        if (tagCounts.GetValueOrDefault("risk") >= 2)
        {
            outcome.Damage += 6;
            outcome.FogGain += 1;
            outcome.Notes.Add(Localization.Language == Localization.English ? "Surge backlash: HP -6, noise +1." : "过载反震：勇气 -6，声响 +1。");
        }
    ...
    private string BuildCalibrationResultText(CalibrationOutcome outcome)
    {
        ...
        if (outcome.Shards > 0)
        {
            parts.Add(Localization.Language == Localization.English ? $"Buttons +{outcome.Shards}" : $"纽扣 +{outcome.Shards}");
        }
        if (outcome.Oil > 0)
        {
            parts.Add(Localization.Language == Localization.English ? $"Battery +{outcome.Oil}" : $"手电 +{outcome.Oil}");
        }
        if (outcome.FogReduction > 0)
        {
            parts.Add(Localization.Language == Localization.English ? $"Noise -{outcome.FogReduction}" : $"声响 -{outcome.FogReduction}");
        }
        if (outcome.Heal > 0)
        {
        ...
        {
            parts.Add(Localization.Language == Localization.English ? $"Noise +{outcome.FogGain}" : $"声响 +{outcome.FogGain}");
        }
    ...
    private static string FormatCalibrationNodePreview(CalibrationNode node)
    {
        ...
        if (node.Shards > 0)
        {
            parts.Add(Localization.Language == Localization.English ? $"+{node.Shards} buttons" : $"+{node.Shards} 纽扣");
        }
        if (node.Oil > 0)
        {
            parts.Add(Localization.Language == Localization.English ? $"+{node.Oil} battery" : $"+{node.Oil} 手电");
        }
        if (node.FogReduction > 0)
        {
            parts.Add(Localization.Language == Localization.English ? $"-{node.FogReduction} noise" : $"-{node.FogReduction} 声响");
        }
        ...
        if (node.FogGain > 0)
        {
            parts.Add(Localization.Language == Localization.English ? $"+{node.FogGain} noise" : $"+{node.FogGain} 声响");
        }
    ...
    private void ApplyBattleStartRelics()
    {
        ...
        _battle.SetInspirationModifiers(
            _run.GetRunEffectValue(_gameData, "inspiration_block_bonus"),
            _run.GetRunEffectValue(_gameData, "inspiration_stagger_bonus"));
    ...
    private void OnCardGuiInput(InputEvent inputEvent, Control button, int handIndex)
    {
    ...
    private void SelectAimingCard(Control button, int handIndex)
    {
        CancelAimingCard(false);
        _aimingCardButton = button;
        _aimingCardIndex = handIndex;
        _aimHoverEnemyIndex = -1;
    ...
    private int FindEnemyTargetAt(Vector2 globalPosition)
    {
        ...
        {
            if (pair.Key < 0 || pair.Key >= _battle.Enemies.Count || !_battle.Enemies[pair.Key].IsAlive)
            {
                continue;
            }
    ...
    private void UpdateAimingFeedback(Vector2 globalMousePosition)
    {
        ...
        var card = _battle.Hand[_aimingCardIndex];
        var valid = IsValidAimingTarget(card, globalMousePosition);
        var hoverEnemy = CardTargetsEnemy(card) ? FindEnemyTargetAt(globalMousePosition) : -1;
        if (hoverEnemy != _aimHoverEnemyIndex)
        {
            _aimHoverEnemyIndex = hoverEnemy;
            RefreshAimingTargetScales();
        }
    ...
    private void UpdateAimingLine(Vector2 globalMousePosition)
    {
        ...
        var card = _battle.Hand[_aimingCardIndex];
        var endPoint = ResolveAimEndPoint(card, globalMousePosition);
        _dragLine.Points = BuildAimCurve(_dragLineStart - rootOrigin, endPoint - rootOrigin);
        _dragLine.DefaultColor = IsValidAimingTarget(card, globalMousePosition)
            ? Color.FromHtml("78d69b")
            : Color.FromHtml("d7b45f");
    }

    private Vector2 ResolveAimEndPoint(CardData card, Vector2 globalMousePosition)
    {
        if (CardTargetsEnemy(card))
        {
            var enemyIndex = FindEnemyTargetAt(globalMousePosition);
            if (enemyIndex >= 0 && _enemyTargetButtons.TryGetValue(enemyIndex, out var target) && GodotObject.IsInstanceValid(target))
            {
                return GetAimPoint(target, 0.42f);
            }
        }
        else if (_playerCombatPanel.GetGlobalRect().HasPoint(globalMousePosition))
        {
            return GetAimPoint(_playerCombatPanel, 0.36f);
        }

        return globalMousePosition;
    }

    private static Vector2 GetAimPoint(Control target, float verticalFactor)
    {
        var rect = target.GetGlobalRect();
        return rect.Position + rect.Size * new Vector2(0.5f, verticalFactor);
    }

    private static Vector2[] BuildAimCurve(Vector2 start, Vector2 end)
    {
        const int segments = 18;
        var points = new Vector2[segments + 1];
        var height = Math.Clamp(Math.Abs(start.X - end.X) * 0.16f + Math.Abs(start.Y - end.Y) * 0.18f + 92f, 92f, 220f);
        var control = new Vector2((start.X + end.X) * 0.5f, Math.Min(start.Y, end.Y) - height);
    ...
    private void RefreshAimingTargetScales()
    {
        foreach (var pair in _enemyTargetButtons)
        {
            ...
            if (pair.Key == _aimHoverEnemyIndex)
            {
                pair.Value.Scale = new Vector2(1.045f, 1.045f);
                pair.Value.Modulate = Color.FromHtml("ffffff");
            }
            else
            {
                pair.Value.Scale = Vector2.One;
                pair.Value.Modulate = pair.Key == _battle.SelectedEnemyIndex ? Color.FromHtml("ffffff") : Color.FromHtml("e9edf2");
            }
        }
    }

    private void ResetAimingTargetScales()
    {
        foreach (var pair in _enemyTargetButtons)
        {
            if (!GodotObject.IsInstanceValid(pair.Value))
    ...
    private BattleValueSnapshot CaptureBattleSnapshot()
    {
        var snapshot = new BattleValueSnapshot
        {
            ...
            InspirationsThisTurn = _battle.InspirationsThisTurn
        };
        ...
        foreach (var enemy in _battle.Enemies)
        {
            ...
            snapshot.EnemyStagger.Add(enemy.Stagger);
    ...
    private void ShowBattleDeltaFeedback(BattleValueSnapshot before, bool includePlayerResourceGains, CardData? playedCard = null)
    {
        ...
        if (includePlayerResourceGains)
        {
            ...
            var inspirationDelta = _battle.InspirationsThisTurn - before.InspirationsThisTurn;
            if (inspirationDelta > 0)
            {
                SpawnFloatingText(_playerCombatPanel, inspirationDelta == 1 ? "灵感!" : $"灵感 x{inspirationDelta}", "draw", playerOffset++);
            }
        }
        ...
        for (var i = 0; i < enemyCount; i++)
        {
            ...
            if (i < before.EnemyStagger.Count)
            {
                var staggerDelta = enemy.Stagger - before.EnemyStagger[i];
                if (staggerDelta > 0)
                {
                    SpawnFloatingText(target, $"+{staggerDelta} 破势", "status", offset++);
                }
                else if (staggerDelta < 0 && enemy.IsAlive)
                {
                    SpawnFloatingText(target, $"{staggerDelta} 破势", "block", offset++);
                }
            }
        }
    }
    ...
    private void ResolveBattleIfFinished()
    {
                ...
                RunLoadingTransition(
                    ...
                    () =>
                    {
                        ...
                        _roomDescriptionLabel.Text = Localization.Language == Localization.English
                            ...
                            : "怪物被击退。你可以继续根据线索探索当前房间，或在找到出口后深入下一层。";
                        RenderMinefield();
                        RenderShared();
                        ShowGameTip(Localization.Language == Localization.English
        ...
        else if (_battle.PlayerHp <= 0)
        {
            ...
            RunLoadingTransition(
                ...
                () => ShowEndPanel(false, "你的勇气用光了。下一次钻进床下前，最好再调整牌组与路线。"));
        }
    }
    ...
    private void ShowBattleReward()
    {
        ...
        _roomDescriptionLabel.Text = Localization.Language == Localization.English
            ? "Choose one spoil. Keepsakes change the rest of this run."
            : "选择一项战利品。纪念物会改变本局后续路线、战斗与资源压力。";
        ShowGameTip(Localization.Language == Localization.English
            ? "Battle won\nChoose a spoil."
            : "战斗胜利\n选择一项战利品。");
        ...
        var offers = BuildBattleRewardOffers(reward);
        var cardOfferGrid = CreateRewardCardGrid("BattleRewardCardGrid", Math.Min(3, Math.Max(1, offers.Count)));
        var hasCardOffers = false;
        foreach (var offer in offers)
        {
            if (offer.Card != null)
            {
                hasCardOffers = true;
                AddBattleRewardCardOffer(reward, offer, cardOfferGrid);
            }
        }

        if (hasCardOffers)
        {
            _rewardList.AddChild(cardOfferGrid);
        }
        ...
        foreach (var offer in offers)
        {
            if (offer.Card == null)
            {
                AddBattleRewardOffer(reward, offer);
            }
        }

        var skipText = Localization.Language == Localization.English
            ? $"Take only the base spoils: {PreviewBaseReward(reward)}."
            : $"只拿基础战利品：{PreviewBaseReward(reward)}。";
        ...
        skipButton.Pressed += () => OnRewardOfferPicked(reward, BattleRewardOffer.BaseOnly());
        _rewardList.AddChild(skipButton);
    }

    private List<BattleRewardOffer> BuildBattleRewardOffers(RewardData reward)
    {
        var result = new List<BattleRewardOffer>();
        var random = new Random(HashCode.Combine(_run.RunSeed, _run.CurrentLayerIndex, _run.BattlesWon, reward.Id, _run.PlayerDeck.Count));
        var targetCount = random.Next(
            Math.Max(1, reward.MinOffers),
            Math.Max(Math.Max(1, reward.MinOffers), reward.MaxOffers) + 1);
        var pickedKinds = new HashSet<string>();
        var remainingBudget = Math.Max(30, reward.ValueBudget);

        while (pickedKinds.Count < targetCount)
        {
            var kind = PickRewardOfferKind(reward, random, pickedKinds, remainingBudget, pickedKinds.Count == 0);
            ...
            remainingBudget -= GetRewardKindValue(reward, kind);
            AddOffersForKind(result, reward, kind, random);
        }

        if (result.Count == 0)
        {
            result.Add(BattleRewardOffer.FromButtons(Math.Max(4, reward.BonusShardsMin)));
        }

        return result;
    }

    private string PickRewardOfferKind(RewardData reward, Random random, HashSet<string> pickedKinds, int remainingBudget, bool allowOverBudget)
    {
        var weighted = new List<(string Kind, int Weight)>();
        AddRewardKind(weighted, reward, pickedKinds, "card", reward.CardOfferWeight, HasCardRewardChoices(reward), remainingBudget, allowOverBudget);
        AddRewardKind(weighted, reward, pickedKinds, "relic", reward.RelicOfferWeight, HasRelicRewardChoices(reward), remainingBudget, allowOverBudget);
        AddRewardKind(weighted, reward, pickedKinds, "supply", reward.SupplyOfferWeight, HasSupplyRewardChoices(reward), remainingBudget, allowOverBudget);
        AddRewardKind(weighted, reward, pickedKinds, "buttons", reward.ButtonOfferWeight, reward.BonusShardsMax > 0, remainingBudget, allowOverBudget);
        AddRewardKind(weighted, reward, pickedKinds, "heal", reward.HealOfferWeight, reward.BonusHealMax > 0 && _run.PlayerHp < _run.PlayerMaxHp, remainingBudget, allowOverBudget);
        ...
        return weighted[0].Kind;
    }

    private static void AddRewardKind(List<(string Kind, int Weight)> weighted, RewardData reward, HashSet<string> pickedKinds, string kind, int weight, bool available, int remainingBudget, bool allowOverBudget)
    {
        if (!available || weight <= 0 || pickedKinds.Contains(kind))
        {
            return;
        }

        var value = GetRewardKindValue(reward, kind);
        if (!allowOverBudget && value > remainingBudget)
        {
            return;
        }

        var adjustedWeight = value > remainingBudget ? Math.Max(1, weight / 3) : weight;
        weighted.Add((kind, adjustedWeight));
    }

    private static int GetRewardKindValue(RewardData reward, string kind)
    {
        return kind switch
        {
            "card" => Math.Max(1, reward.CardOfferValue),
            "relic" => Math.Max(1, reward.RelicOfferValue),
            "supply" => Math.Max(1, reward.SupplyOfferValue),
            "buttons" => Math.Max(1, reward.ButtonOfferValue),
            "heal" => Math.Max(1, reward.HealOfferValue),
            _ => 1
        };
    }

    private bool HasCardRewardChoices(RewardData reward)
    {
        return _gameData.BuildRewardChoices(reward, _run.CharacterId, HashCode.Combine(_run.RunSeed, reward.Id, "card_probe"), 1).Count > 0;
    }

    private bool HasRelicRewardChoices(RewardData reward)
    {
        return _gameData.BuildRelicChoices(reward, _run.Relics, HashCode.Combine(_run.RunSeed, reward.Id, "relic_probe"), 1).Count > 0;
    }

    private static bool HasSupplyRewardChoices(RewardData reward)
    {
        return reward.Supplies.Count > 0;
    }

    private void AddOffersForKind(List<BattleRewardOffer> result, RewardData reward, string kind, Random random)
    {
        switch (kind)
        {
            case "card":
                var cardChoiceCount = Math.Max(1, reward.CardPickCount + Math.Max(0, _run.GetRunEffectValue(_gameData, "reward_card_bonus")));
                var cardChoices = _gameData.BuildRewardChoices(
                    reward,
                    _run.CharacterId,
                    HashCode.Combine(_run.RunSeed, _run.CurrentLayerIndex, reward.Id, _run.PlayerDeck.Count, "card_pack"),
                    cardChoiceCount);
                foreach (var card in cardChoices)
                {
                    result.Add(BattleRewardOffer.FromCard(card));
                }
                break;
            case "relic":
                var relicChoices = _gameData.BuildRelicChoices(
                    reward,
                    _run.Relics,
                    HashCode.Combine(_run.RunSeed, _run.CurrentLayerIndex, reward.Id, "relic_pack"),
                    Math.Max(1, reward.RelicPickCount));
                foreach (var relic in relicChoices)
                {
                    result.Add(BattleRewardOffer.FromRelic(relic));
                }
                break;
            case "supply":
                var supply = PickSupplyReward(reward, random);
                if (supply != null)
                {
                    result.Add(BattleRewardOffer.FromSupply(supply.ItemId, Math.Max(1, supply.Count)));
                }
                break;
            case "buttons":
                result.Add(BattleRewardOffer.FromButtons(RollRange(random, reward.BonusShardsMin, reward.BonusShardsMax)));
                break;
            case "heal":
                result.Add(BattleRewardOffer.FromHeal(RollRange(random, reward.BonusHealMin, reward.BonusHealMax)));
                break;
        }
    }

    private static int RollRange(Random random, int min, int max)
    {
    ...
    private static RewardSupplyData? PickSupplyReward(RewardData reward, Random random)
    {
        if (reward.Supplies.Count == 0)
        {
            return null;
        }

        var total = 0;
        foreach (var supply in reward.Supplies)
        {
            total += Math.Max(1, supply.Weight);
        }

        var roll = random.Next(Math.Max(1, total));
        foreach (var supply in reward.Supplies)
        {
            roll -= Math.Max(1, supply.Weight);
            if (roll < 0)
            {
                return supply;
            }
        ...
        return reward.Supplies[0];
    }

    private void AddBattleRewardOffer(RewardData reward, BattleRewardOffer offer)
    {
        if (offer.Card != null)
        {
            AddBattleRewardCardOffer(reward, offer, _rewardList);
            return;
        }

        var button = new Button
        {
            Text = FormatRewardOfferText(offer),
            CustomMinimumSize = new Vector2(0, offer.Relic != null ? 74 : 62),
            AutowrapMode = TextServer.AutowrapMode.WordSmart
        };
        StyleButton(button, GetRewardOfferColor(offer), GetRewardOfferFontColor(offer));
        button.TooltipText = FormatRewardOfferTooltip(offer);
        button.Pressed += () => OnRewardOfferPicked(reward, offer);
        _rewardList.AddChild(button);
    }

    private void AddBattleRewardCardOffer(RewardData reward, BattleRewardOffer offer, Container parent)
    {
        if (offer.Card == null)
        {
            return;
        ...
        var footer = Localization.Language == Localization.English
            ? "Take this card"
            : "选择这张卡";
        var cell = CardCell.Create(offer.Card, CardCellMode.Deck, true, footer);
        cell.TooltipText = FormatRewardOfferTooltip(offer);
        cell.MouseDefaultCursorShape = CursorShape.PointingHand;
        cell.GuiInput += inputEvent =>
        {
            if (inputEvent is InputEventMouseButton { Pressed: true, ButtonIndex: MouseButton.Left })
            {
                OnRewardOfferPicked(reward, offer);
            }
        };
        parent.AddChild(cell);
    }

    private string FormatRewardOfferText(BattleRewardOffer offer)
    {
        if (offer.Relic != null)
        {
            return $"{FormatRelicReward(offer.Relic)}\n{RewardValueLine(Localization.Language == Localization.English ? "Long-term power" : "长期成长", "S")}";
        }

        if (!string.IsNullOrWhiteSpace(offer.ItemId))
        {
            var item = _gameData.GetItem(offer.ItemId);
            return Localization.Language == Localization.English
                ? $"Supply · {item.DisplayName()} x{offer.ItemCount}\n{item.DisplayDescription()}\n{RewardValueLine("Immediate utility", "B")}"
                : $"补给 · {item.DisplayName()} x{offer.ItemCount}\n{item.DisplayDescription()}\n{RewardValueLine("即时工具", "B")}";
        }

        if (offer.BonusShards > 0)
        {
            return Localization.Language == Localization.English
                ? $"Button pouch\nGain +{offer.BonusShards} extra buttons.\n{RewardValueLine("Stable resource", "B")}"
                : $"纽扣袋\n额外获得 +{offer.BonusShards} 纽扣。\n{RewardValueLine("稳定资源", "B")}";
        }

        if (offer.BonusHeal > 0)
        {
            return Localization.Language == Localization.English
                ? $"Catch your breath\nRestore +{offer.BonusHeal} extra courage.\n{RewardValueLine("Survival value", "A")}"
                : $"喘口气\n额外恢复 +{offer.BonusHeal} 点勇气。\n{RewardValueLine("生存价值", "A")}";
        }

        return Localization.Language == Localization.English
            ? $"Base spoils\n{PreviewBaseReward(_gameData.GetReward(_activeBattleRoom?.RewardId ?? "basic"))}"
            : $"基础战利品\n{PreviewBaseReward(_gameData.GetReward(_activeBattleRoom?.RewardId ?? "basic"))}";
    }

    private static string RewardValueLine(string label, string grade)
    {
        return Localization.Language == Localization.English
            ? $"Value: {label} · {grade}"
            : $"价值：{label} · {grade}";
    ...
    private string FormatRewardOfferTooltip(BattleRewardOffer offer)
    {
        if (offer.Card != null)
        {
            return Localization.Language == Localization.English
                ? $"Deck growth · A\n{offer.Card.DisplayName()}\n{offer.Card.DisplayDescription()}"
                : $"牌组成长 · A\n{offer.Card.DisplayName()}\n{offer.Card.DisplayDescription()}";
        }

        return FormatRewardOfferText(offer);
    }

    private string PreviewBaseReward(RewardData reward)
    {
        ...
        return Localization.Language == Localization.English
            ? $"{shardPreview} buttons, {reward.Heal} courage"
            : $"{shardPreview} 纽扣，{reward.Heal} 勇气";
    }

    private static Color GetRewardOfferColor(BattleRewardOffer offer)
    {
        if (offer.Relic != null)
        {
            return Color.FromHtml("392d4b");
        }

        if (!string.IsNullOrWhiteSpace(offer.ItemId))
        {
            return Color.FromHtml("2f4c54");
        }

        if (offer.BonusHeal > 0)
        {
            return Color.FromHtml("2e4a3b");
        }

        return Color.FromHtml("3a3140");
    }

    private static Color GetRewardOfferFontColor(BattleRewardOffer offer)
    {
        return offer.Relic != null ? Color.FromHtml("f0e4ff") : Color.FromHtml("d8e2ee");
    }

    private void OnRewardOfferPicked(RewardData reward, BattleRewardOffer offer)
    {
        var before = CaptureRunTipSnapshot();
        _run.ApplyRewardPackage(reward, offer.Card, offer.Relic, _gameData, _activeBattleRoom?.RewardBonus ?? 0, offer.BonusShards, offer.BonusHeal, offer.ItemId, offer.ItemCount);
        if (offer.Card != null)
        {
            ShowGameTip(Localization.Language == Localization.English
                ? $"Card reward\n{offer.Card.DisplayName()}: {offer.Card.DisplayDescription()}"
                : $"卡牌奖励\n{offer.Card.DisplayName()}：{offer.Card.DisplayDescription()}");
        }
        if (offer.Relic != null)
        {
            ShowGameTip(Localization.Language == Localization.English
                ? $"Keepsake reward\n{offer.Relic.DisplayName()}: {offer.Relic.DisplayDescription()}"
                : $"纪念物奖励\n{offer.Relic.DisplayName()}：{offer.Relic.DisplayDescription()}");
        }
        ShowRunDeltaTips(before);
    ...
    private static string FormatRelicReward(RelicData relic)
    {
        ...
        {
            "rare" => Localization.Language == Localization.English ? "Rare Keepsake" : "稀有纪念物",
            "uncommon" => Localization.Language == Localization.English ? "Uncommon Keepsake" : "罕见纪念物",
            _ => Localization.Language == Localization.English ? "Common Keepsake" : "普通纪念物"
        };
    ...
    private void ShowBattleRewardLegacy()
    {
        ...
        _roomDescriptionLabel.Text = "从散落的补给里挑选战利品。";
        ...
        foreach (var card in cardChoices)
        {
            var cell = CardCell.Create(card, CardCellMode.Reward, true, Localization.Language == Localization.English ? "Take this card" : "选择这张卡");
            cell.GuiInput += inputEvent =>
            {
                if (inputEvent is InputEventMouseButton { Pressed: true, ButtonIndex: MouseButton.Left })
                {
                    OnRewardPicked(reward, card);
                }
            };
    ...
    private void RenderBattle()
    {
        ...
        var playerStatuses = _battle.PlayerWeak > 0
            ? isEnglish ? $" | Weak {_battle.PlayerWeak}" : $" | 虚弱 {_battle.PlayerWeak}"
            : string.Empty;
        _playerHpBar.MaxValue = _battle.PlayerMaxHp;
        _playerHpBar.Value = Math.Max(0, _battle.PlayerHp);
        _playerCombatLabel.Text = isEnglish
            ? $"Kid Explorer\nHP {_battle.PlayerHp}/{_battle.PlayerMaxHp}  Energy {_battle.Energy}{playerStatuses}"
            : $"小小探险家\n勇气 {_battle.PlayerHp}/{_battle.PlayerMaxHp}  能量 {_battle.Energy}{playerStatuses}";
        _playerCombatPanel.TooltipText = BuildPlayerStatusTooltip();
        _enemyCombatPanel.TooltipText = isEnglish
            ? $"Enemies alive {CountAliveEnemies()}/{_battle.Enemies.Count}. Current target: {_battle.Enemy.DisplayName()}."
            : $"敌人存活 {CountAliveEnemies()}/{_battle.Enemies.Count}。当前目标：{_battle.Enemy.DisplayName()}。";
        _playerBlockLabel.Text = isEnglish ? $"Block {_battle.PlayerBlock}" : $"格挡 {_battle.PlayerBlock}";
        var debtText = _battle.InspirationDebt > 0
            ? isEnglish ? $" | Interference {_battle.InspirationDebt}" : $" | 阻滞 {_battle.InspirationDebt}"
            : string.Empty;
        _battleResourceLabel.Text = isEnglish
            ? $"Energy {_battle.Energy}/3     Draw {_battle.DrawPile.Count}     Discard {_battle.DiscardPile.Count}     Hand {_battle.Hand.Count}     Inspiration {_battle.InspirationProgress}/{_battle.InspirationThreshold}{debtText}"
            : $"能量 {_battle.Energy}/3     抽牌堆 {_battle.DrawPile.Count}     弃牌堆 {_battle.DiscardPile.Count}     手牌 {_battle.Hand.Count}     灵感 {_battle.InspirationProgress}/{_battle.InspirationThreshold}{debtText}";
        ...
        for (var i = 0; i < _battle.Hand.Count; i++)
        {
            ...
            var button = CardCell.Create(card, CardCellMode.Hand, card.Cost <= _battle.Energy, "", i);
    ...
    private void RenderEnemyTargets()
    {
        ...
        for (var i = 0; i < _battle.Enemies.Count; i++)
        {
            ...
            var button = CreateEnemyTargetButton(enemy, i, selected);
    ...
    private Button CreateEnemyTargetButton(BattleEnemyState enemy, int index, bool selected)
    {
        var button = new Button
        {
            Text = string.Empty,
            CustomMinimumSize = new Vector2(GetEnemyTargetWidth(), 278),
            SizeFlagsHorizontal = SizeFlags.ExpandFill,
        ...
        layout.AddChild(MakeEnemyLabel(BuildEnemyIntentBadge(index, enemy), 14, enemy.IsAlive ? Color.FromHtml("ffe8bd") : Color.FromHtml("687381"), HorizontalAlignment.Center, true));
        layout.AddChild(MakeEnemyLabel(
            selected ? Localization.Language == Localization.English ? "TARGET" : "当前目标" : " ",
            12,
            selected ? Color.FromHtml("8df0bd") : Color.FromHtml("47515d"),
            HorizontalAlignment.Center,
            false));
        layout.AddChild(MakeEnemyLabel(GetEnemyPortraitGlyph(enemy), enemy.IsAlive ? 34 : 28, enemy.IsAlive ? Color.FromHtml("f0e4ff") : Color.FromHtml("687381"), HorizontalAlignment.Center, false));
        layout.AddChild(MakeEnemyLabel(enemy.Data.DisplayName(), 18, enemy.IsAlive ? Color.FromHtml("ffe2e6") : Color.FromHtml("7b8490"), HorizontalAlignment.Center, true));

        var hpBar = MakeEnemyProgressBar(Math.Max(0, enemy.Hp), Math.Max(1, enemy.MaxHp), Color.FromHtml("c95765"));
        hpBar.TooltipText = $"HP {Math.Max(0, enemy.Hp)}/{enemy.MaxHp}";
        layout.AddChild(hpBar);

        layout.AddChild(MakeEnemyLabel(
            enemy.IsAlive
                ? $"HP {Math.Max(0, enemy.Hp)}/{enemy.MaxHp}     {(Localization.Language == Localization.English ? "Block" : "格挡")} {enemy.Block}"
                : (Localization.Language == Localization.English ? "Defeated" : "已击倒"),
            13,
            enemy.IsAlive ? Color.FromHtml("d8e2ee") : Color.FromHtml("6e7884"),
            HorizontalAlignment.Center,
            false));

        var staggerBar = MakeEnemyProgressBar(Math.Max(0, enemy.Stagger), Math.Max(1, enemy.StaggerLimit), Color.FromHtml("9d75e5"));
        staggerBar.TooltipText = Localization.Language == Localization.English
            ? $"Stagger {enemy.Stagger}/{enemy.StaggerLimit}"
            : $"破势 {enemy.Stagger}/{enemy.StaggerLimit}";
        layout.AddChild(staggerBar);

        var status = FormatEnemyStatusLine(enemy);
        if (!string.IsNullOrWhiteSpace(status))
        {
            layout.AddChild(MakeEnemyLabel(status.Replace("\n", " "), 12, Color.FromHtml("c995ff"), HorizontalAlignment.Center, true));
        }

        layout.AddChild(MakeEnemyLabel(BuildEnemyCounterHint(_battle.GetCurrentEnemyIntent(index)), 12, Color.FromHtml("9fabb8"), HorizontalAlignment.Center, true));
    ...
    private float GetEnemyTargetWidth()
    {
        return _battle.Enemies.Count switch
        {
            <= 1 => 330f,
            2 => 244f,
            3 => 198f,
    ...
    private static Label MakeEnemyLabel(string text, int fontSize, Color color, HorizontalAlignment alignment, bool wrap)
    {
        var label = new Label
        {
            Text = text,
    ...
    private static ProgressBar MakeEnemyProgressBar(int value, int maxValue, Color color)
    {
        var bar = new ProgressBar
        {
            MinValue = 0,
            MaxValue = Math.Max(1, maxValue),
            Value = Math.Clamp(value, 0, Math.Max(1, maxValue)),
            ShowPercentage = false,
            CustomMinimumSize = new Vector2(0, 12),
            MouseFilter = MouseFilterEnum.Ignore
        };
        StyleProgressBar(bar, color);
        return bar;
    }

    private string BuildEnemyIntentBadge(int index, BattleEnemyState enemy)
    {
        if (!enemy.IsAlive)
        {
            return Localization.Language == Localization.English ? "Defeated" : "已击倒";
        }

        var intent = _battle.GetCurrentEnemyIntent(index);
        var icon = GetIntentIcon(intent);
        return $"{icon} {intent.DisplayName()}  |  {_battle.GetEnemyIntentPreview(index)}";
    }

    private static string GetIntentIcon(IntentData intent)
    {
        var hasDamage = false;
        var hasBlock = false;
    ...
    private static string GetEnemyPortraitGlyph(BattleEnemyState enemy)
    {
        if (!enemy.IsAlive)
        {
            return "X";
        ...
        if (enemy.Data.Tags.Contains("swarm") || enemy.Data.Id.Contains("mite"))
        {
            return "虫";
        }
        if (enemy.Data.Tags.Contains("guardian") || enemy.Data.Id.Contains("guard"))
        {
            return "盾";
        }
        if (enemy.Data.Tags.Contains("boss") || enemy.Data.Id.Contains("core"))
        {
            return "王";
        }
    ...
    private string FormatEnemyTargetText(BattleEnemyState enemy, int index, bool selected)
    {
        ...
        var testType = FormatEnemyTestType(enemy.Data);
        var testLine = string.IsNullOrWhiteSpace(testType) ? string.Empty : $"{testType}\n";
        return $"{marker}{enemy.Data.DisplayName()}\n{testLine}HP {Math.Max(0, enemy.Hp)}/{enemy.MaxHp}  格挡 {enemy.Block}\n破势 {enemy.Stagger}/{enemy.StaggerLimit}{status}\n下一步: {intentPreview}\n应对: {counterHint}";
    }
    ...
    private static string BuildEnemyCounterHint(IntentData intent)
    {
        ...
        if (hasControl)
        {
            return Localization.Language == Localization.English ? "Protect your rhythm or switch target." : "它会干扰节奏，考虑转火或提前触发灵感。";
        }
    ...
    {
        return Localization.Language == Localization.English
            ? "Status: every 3 Inspiration progress draws 1 card, gains block, and staggers all enemies. Weak reduces outgoing attack damage by 2. Vulnerable takes 50% more player attack damage."
            : "状态说明：每累计 3 点灵感，会抽 1 张牌、获得格挡，并使所有敌人破势。虚弱使攻击伤害 -2；易伤使受到的玩家攻击伤害 +50%。";
    }
    ...
    private string BuildPlayerStatusTooltip()
    {
        var lines = new List<string>
        {
            Localization.Language == Localization.English
                ? $"Inspiration {_battle.InspirationProgress}/{_battle.InspirationThreshold}: every 3 progress draws 1 card, gains block, and staggers all enemies."
                : $"灵感 {_battle.InspirationProgress}/{_battle.InspirationThreshold}：每累计 3 点会抽 1 张牌、获得格挡，并使所有敌人破势。"
        };
        if (_battle.InspirationDebt > 0)
        {
            lines.Add(Localization.Language == Localization.English
                ? $"Interference {_battle.InspirationDebt}: upcoming Inspiration progress is cancelled first."
                : $"阻滞 {_battle.InspirationDebt}：接下来的灵感推进会先被抵消。");
        }
    ...
    private string BuildCardTooltip(CardData card)
    {
        ...
        foreach (var action in card.Actions)
        {
            switch (action.Type)
            {
                ...
                case "combo_damage":
                    lines.Add(Localization.Language == Localization.English
                        ? "Rhythm attack: gains damage for each card already played this turn."
                        : "节奏攻击：本回合已打出的牌越多，伤害越高。");
                    break;
                case "combo_block":
                    lines.Add(Localization.Language == Localization.English
                        ? "Rhythm block: gains block for each card already played this turn."
                        : "节奏格挡：本回合已打出的牌越多，格挡越高。");
                    break;
                case "inspire":
                    lines.Add(Localization.Language == Localization.English
                        ? "Inspire: advances the 3-step Inspiration meter."
                        : "激发：推进灵感计量。");
                    break;
                case "stagger":
                case "stagger_all":
                    lines.Add(Localization.Language == Localization.English
                        ? "Stagger: filling an enemy's stagger bar applies Weak and Vulnerable."
                        : "破势：填满敌人的破势条会施加虚弱和易伤。");
                    break;
            }
        }
    ...
    private void LayoutHandFan()
    {
        ...
        var availableSize = new Vector2(
            ...
            Math.Max(Math.Max(_handBox.Size.Y, parentSize.Y), 226f));
        ...
        var spacing = count <= 1
            ? 0f
            : Math.Min(cardSize.X * 0.72f, Math.Max(64f, (availableSize.X - cardSize.X) / (count - 1)));
        var totalWidth = cardSize.X + spacing * Math.Max(0, count - 1);
        var startX = Math.Max(0, (availableSize.X - totalWidth) / 2f);
        var baseY = Math.Max(6f, availableSize.Y - cardSize.Y - 56f);
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            cardControl.RotationDegrees = normalized * 11f;
            cardControl.Position = new Vector2(startX + spacing * i, baseY + Math.Abs(normalized) * 28f);
        }
    }
    ...
    {
        ClearBox(_itemList);
        AddInventoryHeader(Localization.Language == Localization.English ? "Keepsakes" : "纪念物");
        if (_run.Relics.Count == 0)
        {
            AddInventoryEmpty(Localization.Language == Localization.English ? "No keepsakes yet." : "暂无纪念物。");
        }
        else
        {
    ...
    {
        _roomTitleLabel.Text = Localization.Language == Localization.English ? "Keepsake Details" : "纪念物详情";
    ...
    private static string FormatRelicEffect(RelicEffectData effect)
    {
        ...
        return effect.Type switch
        {
            ...
            "reward_shards_percent" => Localization.Language == Localization.English ? $"Battle reward buttons +{value}%." : $"战斗奖励纽扣 +{value}%。",
            "reward_card_bonus" => Localization.Language == Localization.English ? $"Card reward choices +{value}." : $"卡牌奖励选项 +{value}。",
            "mine_start_preview" => Localization.Language == Localization.English ? $"Preview {value} room tile at search start." : $"进入房间时预览 {value} 个房间格子。",
            "post_battle_fog_reduction" => Localization.Language == Localization.English ? $"After battle: noise -{value}." : $"战斗后：声响 -{value}。",
            "post_battle_lamp" => Localization.Language == Localization.English ? $"After battle: battery +{value}." : $"战斗后：手电 +{value}。",
            "max_lamp" => Localization.Language == Localization.English ? $"Max battery +{value}." : $"手电上限 +{value}。",
            ...
            "inspiration_block_bonus" => Localization.Language == Localization.English ? $"Inspiration block +{value}." : $"灵感触发格挡 +{value}。",
            "inspiration_stagger_bonus" => Localization.Language == Localization.English ? $"Inspiration stagger +{value}." : $"灵感触发破势 +{value}。",
    ...
    private static string FormatItemUseHint(ItemData item)
    {
        return item.UseMode switch
        {
            ...
            "target_tile" => Localization.Language == Localization.English ? "Click to select, then choose a room tile." : "点击选中后，再选择一个房间格子。",
    ...
    private static void ClearBox(Node container)
    {
    ...
    private static GridContainer CreateRewardCardGrid(string name, int columns)
    {
        var grid = new GridContainer
        {
            Name = name,
    ...
    {
        var lines = new List<string>();
        AddDeltaLine(lines, Localization.Language == Localization.English ? "Courage" : "勇气", _run.PlayerHp - before.Hp);
        AddDeltaLine(lines, Localization.Language == Localization.English ? "Buttons" : "纽扣", _run.Shards - before.Shards);
        AddDeltaLine(lines, Localization.Language == Localization.English ? "Battery" : "手电", _run.LampOil - before.LampOil);
        AddDeltaLine(lines, Localization.Language == Localization.English ? "Noise" : "声响", _run.FogPressure - before.FogPressure);
    ...
    private void BuildModalHost()
    {
        ...
        _dragLine.Antialiased = true;
        _dragLine.BeginCapMode = Line2D.LineCapMode.Round;
        _dragLine.EndCapMode = Line2D.LineCapMode.Round;
    ...
    private async void PlayNextGameTip()
    {
        ...
        var height = 35f;
        var target = new Vector2(0f, Math.Max(92f, root.Size.Y * 0.14f));
        tip.Size = new Vector2(width, height);
        tip.Position = target + new Vector2(0, -12f);
        ...
        tween.TweenProperty(tip, "position", target + new Vector2(0, -12f), 0.45f).SetTrans(Tween.TransitionType.Sine).SetEase(Tween.EaseType.InOut);
    ...
    private void ApplyUiStyle()
    {
        ...
        StyleButton(_infoToggleButton, Color.FromHtml("2f4c54"), Color.FromHtml("e4fbff"));
    ...
    private static StyleBoxFlat MakeTopTipStyle()
    {
        ...
        style.SetBorderWidth(Side.Top, 1);
    ...
    private static void StyleRouteNodeButton(Button button, RunRoom room, bool current, bool selectable, bool past, bool onRoute)
    {
        ...
        if (past)
        {
            background = onRoute ? Color.FromHtml("141b22") : Color.FromHtml("10151b");
            border = onRoute ? Color.FromHtml("33414c") : Color.FromHtml("222a33");
            font = onRoute ? Color.FromHtml("758291") : Color.FromHtml("4d5865");
            borderWidth = 1;
        }
        else if (!onRoute)
        {
    ...
    private static Color GetRouteKindColor(string kind)
    {
        return kind switch
        {
            ...
            "minigame" => Color.FromHtml("b58cff"),
    ...
    private sealed class BattleValueSnapshot
    {
        ...
        public int InspirationsThisTurn { get; set; }
        public List<int> EnemyHp { get; } = new();
        public List<int> EnemyBlock { get; } = new();
        public List<int> EnemyStagger { get; } = new();
    ...
    private sealed class BattleRewardOffer
    {
        ...
        public static BattleRewardOffer BaseOnly() => new();

        public static BattleRewardOffer FromCard(CardData card) => new() { Card = card };

        public static BattleRewardOffer FromRelic(RelicData relic) => new() { Relic = relic };

        public static BattleRewardOffer FromSupply(string itemId, int count) => new()
        {
            ItemId = itemId,
            ItemCount = count
        };

        public static BattleRewardOffer FromButtons(int amount) => new()
        {
            BonusShards = amount
        };

        public static BattleRewardOffer FromHeal(int amount) => new()
        {
            BonusHeal = amount
        };
    }
